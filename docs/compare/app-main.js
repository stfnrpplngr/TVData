...
const STATIC_REMOTE_TABLE_BASE_CANDIDATES = [
...
function localTableBaseCandidates() {
  ...
  return unique([
    ...
    '/tables',
    `${repoRoot}/tables`,
    ...relativePaths.map((x) => `${origin}${x}`),
    `${origin}/tables`,
    `${origin}${repoRoot}/tables`,
  ]);
}

function githubRemoteTableCandidates() {
  const host = window.location.hostname;
  if (!host.endsWith('github.io')) return [];
  const owner = host.split('.')[0];
  const segments = window.location.pathname.split('/').filter(Boolean);
  const repo = segments[0];
  if (!owner || !repo || repo === 'docs') return [];

  const refs = ['main', 'master', 'gh-pages'];
  const raw = refs.map((ref) => `https://raw.githubusercontent.com/${owner}/${repo}/${ref}/tables`);
  const jsdelivr = refs.map((ref) => `https://cdn.jsdelivr.net/gh/${owner}/${repo}@${ref}/tables`);
  return unique([...raw, ...jsdelivr]);
}

function tableBaseCandidates() {
  return unique([...localTableBaseCandidates(), ...githubRemoteTableCandidates(), ...STATIC_REMOTE_TABLE_BASE_CANDIDATES]);
}
